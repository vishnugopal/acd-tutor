...
`src/console/components/TextInput.tsx` → `test/console/components/TextInput.test.ts`). They are
...
5. **Console frontend** — `src/console/` (Ink/React): generic `runConsole(options)` (the `index.tsx` shell) — renders an arrow-key agent picker (`components/Menu.tsx`, skipped when there's one agent) then the chat (`components/App.tsx`): streamed-output rendering, action buttons, thinking indicator, error display. Layout mirrors the web client: Ink widgets live in `components/`, the send loop in `hooks/useChatStream.ts`, and `actions.ts` + `types.ts` stay at the `src/console/` root alongside `index.tsx`/`main.ts`. Knows nothing about the tutors or Flue; it takes `agents: AgentChoice[]` and `createReply(id)`, and each choice carries its own greeting/farewell/actions. (`AgentChoice`/`ReplyChunk` are aliases of the shared `AgentPresentation`/`AgentChunk`.)
...
- **Web client decomposition** — autosave is a pure reducer (`client/lib/autosave.ts`) driven by `client/hooks/useAutosave.ts` (debounce, one auto-retry on failure, `flush()` returns false on a failed write so callers never discard unsaved work); tutor-driven file sync is the pure `planSync` decision tree + hook in `client/hooks/useLessonFileSync.ts`; both lesson screens share `client/components/LessonScreenShell.tsx` (AppBar + options menu: debug-mode toggle + reset confirm). The reply-stream fold calculations live in `src/agents/io/stream-fold.ts`, consumed by both `src/console/hooks/useChatStream.ts` (queue pump + hook) and `src/web/client/hooks/useAgentChat.ts`; the web debug-mode toggle in the options menu is the counterpart of the console's `/debug`. Both frontends queue input sent while a reply streams (FIFO, nothing dropped): the console keeps the queue inline in `useChatStream.ts`, while the web extracts it to the pure `client/lib/messageQueue.ts` (`queueMessage`/`takeNext`) driven by the pump effect in `useAgentChat.ts`, with queued messages rendered as grayed-out bubbles in `ChatPanel.tsx`.
