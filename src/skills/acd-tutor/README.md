...
- **Storage-agnostic**: the tutor only sees four host-provided tools — `listFiles()`, `readFile()`, `writeFile()`, `openFile()` (defined in `src/shared/tools.ts`). Where files live (`ACD_TUTOR_SCRATCH_DIR`, default `/tmp/acd-tutor/scratch`) and which editor opens them (`$EDITOR`) are host-side agent configuration in `src/agents/profiles/acd-tutor.ts`.
