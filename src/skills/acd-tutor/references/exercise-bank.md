...
Each lesson gets a fresh file, `lesson-N.ts`, written verbatim from the code blocks below (including the header comment) via the `writeFile` tool, then shown to the learner with `openFile`.
